...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.layout.setSizeConstraint(QLayout.SetFixedSize) # The layout wont space out when the window is resized

        self.input_display = QLineEdit()
        self.input_display.setClearButtonEnabled(True)
        self.input_display.setValidator(QDoubleValidator()) # Make it so that it accept only numbers (signed/unsigned decimals)
        ...
        for button, pos in zip(num_buttons, num_positions):
            ...
            num_btn.clicked.connect(lambda checked=False,text=button: self.num_button_clicked(text)) 
